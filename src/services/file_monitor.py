...
class VideoFileHandler(FileSystemEventHandler):
    ...
    def __init__(self, callback: Callable[[str, str], None]):
        ...
        self.supported_formats = (
            get_supported_video_formats() + get_supported_audio_formats()
        )
...
class FileMonitor:
    ...
    def _init_drive_service(self):
        ...
        if not client_id or not client_secret:
            logger.warning(
                "Google Drive input monitoring disabled: missing client credentials"
            )
            return
    ...
    async def _update_known_drive_files(self, folder_id: str):
        ...
        try:
            ...
            self.known_drive_files = {
                file["id"] for file in files if self._is_video_file(file)
            }
            logger.debug(
                f"Found {len(self.known_drive_files)} existing files in Google Drive"
            )
    ...
    async def _download_drive_file(self, file_info: dict) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                f"Failed to download Google Drive file {file_info['name']}: {e}"
            )
            return None
