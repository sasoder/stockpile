...
class BRollProcessor:
    ...
    def start(self) -> None:
        ...
        for job in self.job_queue[
            ...
            asyncio.create_task(self.process_video(job.file_path))
    ...
    def _handle_new_file(self, file_path: str, source: str) -> None:
        ...
        self.create_job(file_path, source)
    ...
    def resume_incomplete_jobs(self) -> None:
        ...
        for job in incomplete_jobs:
            ...
            if job.status in [
                ...
                JobStatus.EXTRACTING_PHRASES,
                JobStatus.SEARCHING_YOUTUBE,
    ...
    async def _execute_pipeline(self, job: ProcessingJob) -> None:
        ...
        project_dir = await self._create_project_structure(
            job.job_id, source_filename, search_phrases
        )
        ...
        await self.send_notification(
            job.job_id, "completed", "Job completed successfully"
        )
    ...
    async def evaluate_videos(
        ...
        logger.info(
            f"Selected {len(limited_videos)} top-rated videos for phrase: {phrase}"
        )
        return limited_videos

    async def download_videos(
        self, videos: List[ScoredVideo], phrase: str
    ) -> List[str]:
        ...
        loop = asyncio.get_event_loop()
        downloaded_files = await loop.run_in_executor(
            None, self.video_downloader.download_videos, videos, phrase
        )
    ...
    async def send_notification(self, job_id: str, status: str, message: str) -> None:
        ...
        video_count = None
        if job and job.downloaded_files:
            video_count = sum(len(files) for files in job.downloaded_files.values())
