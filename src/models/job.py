...
class ProcessingJob:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "search_phrases": (
                json.dumps(self.search_phrases) if self.search_phrases else None
            ),
            "downloaded_files": (
                json.dumps(self.downloaded_files) if self.downloaded_files else None
            ),
    ...
    def from_dict(cls, data: dict) -> "ProcessingJob":
        ...
        return cls(
            ...
            search_phrases=(
                json.loads(data["search_phrases"])
                if data.get("search_phrases")
                else None
            ),
            downloaded_files=(
                json.loads(data["downloaded_files"])
    ...
    def update_status(
        self, new_status: JobStatus, error_message: Optional[str] = None
    ) -> None:
